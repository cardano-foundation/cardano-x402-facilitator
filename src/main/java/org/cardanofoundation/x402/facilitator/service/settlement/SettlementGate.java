...
/**
 * Fast health gate for {@code /settle} (spec section 13). Settlement submits a
 * transaction and then confirms its on-chain inclusion; if the chain backend is
 * unreachable the facilitator is blind and MUST NOT accept the settlement — the
 * controller turns a closed gate into HTTP 503 (retryable) rather than a 200 with
 * an ambiguous result. Verification stays in-band (returns
